...
PROJECT_NAME=v2-solucoes
STACK_NAME=v2solucoes-com-br
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://v2-solucoes.localhost"
...
acceptance-frontend-image-build:
	...
	@docker build frontend -t lflrocha/v2-solucoes-frontend:acceptance -f frontend/Dockerfile --build-arg VOLTO_VERSION=$(VOLTO_VERSION)

.PHONY: acceptance-backend-image-build
acceptance-backend-image-build:
	@echo "Build acceptance backend image"
	@docker build backend -t lflrocha/v2-solucoes-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start:
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name v2-solucoes-frontend-acceptance --link v2-solucoes-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d lflrocha/v2-solucoes-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start:
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name v2-solucoes-backend-acceptance -d lflrocha/v2-solucoes-backend:acceptance
...
acceptance-containers-stop: ## Stop Acceptance containers
	...
	@docker stop v2-solucoes-frontend-acceptance
	@docker stop v2-solucoes-backend-acceptance
